...
VOID CCore::Start() {
	...
	if (!Core->Initialise()) {
		Core->Panic("Failed to initialize", "...\\Randomiser\\Core\\Core.cpp", FE_InitFailed, 1);
		int3
	};
...
BOOL CCore::Initialise() {
	...
	if (CheckOldApFile()) {
		//TODO Determine best versioning practice. (Major.Minor.Hotfix)?
		printf_s("The AP.json file is not supported in this version, make sure to finish your previous seed on version <X.X> or use this version on the new Archipelago server\n\n");
	}
